...
<template>
  ...
  <!-- 登录/注册：无导航壳（对应原 layout: false） -->
  <RouterView v-if="route.meta.bare" />

  <div v-else class="min-h-screen flex flex-col" style="font-family: var(--font)">
    <!-- 毛玻璃导航栏 -->
    <header class="topnav">
      <RouterLink to="/" class="topnav__brand">
    ...
    <div v-if="mobileMenuOpen" class="md:hidden border-b border-line bg-white">
      <nav class="px-4 py-3 space-y-1">
        ...
        <div class="pt-2 mt-2 border-t border-line">
          ...
          <template v-else>
            <RouterLink to="/login" class="block px-3 py-2 text-sm hover:bg-bg-muted rounded-md" @click="mobileMenuOpen = false">
              登录
            </RouterLink>
            <RouterLink to="/register" class="block px-3 py-2 text-sm text-accent hover:bg-accent-soft rounded-md" @click="mobileMenuOpen = false">
              注册
            </RouterLink>
          </template>
        </div>
    ...
    <!-- 主内容区（workflow 页面保留组件状态，对应原 keepalive: true） -->
    <main class="flex-1">
      <RouterView v-slot="{ Component }">
        <KeepAlive include="WorkflowView">

...
本项目面向垂直领域（CNC 数控加工）的个性化学习需求，采用 **7 Agent 协同架构**，通过 LangGraph 有向图编排、辩论-裁判质量保障、RAG 知识溯源、苏格拉底式追问等策略，为不同水平的学习者自动生成定制化学习资源。

### 核心指标

| 指标 | 目标 | 验证方法 |
|------|------|----------|
| 知识谬误率 | < 5% | 独立事实核查 — RAG 知识库逐条比对 |
...
┌─────────────────────────────────────────────────────────┐
│                      前端展示层 (Nuxt 3)                   │
│  首页 │ 学情画像 │ Agent协同 │ 资源展示 │ 答题反馈 │ 分析报告  │
├─────────────────────────────────────────────────────────┤
│                   API 网关层 (FastAPI)                     │
├─────────────────────────────────────────────────────────┤
│              多 Agent 协同编排层 (LangGraph)                 │
│  学情诊断 → 路径规划 → 知识生成 → 预审 → 辩论 → 裁判 → 决策   │
├─────────────────────────────────────────────────────────┤
│                      基础设施层                             │
│  LLM (DeepSeek) │ ChromaDB │ MySQL │ Redis │ Celery      │
└─────────────────────────────────────────────────────────┘
```
...
| 知识生成 Agent | 依托 RAG 知识库生成个性化资源 | 检索增强、知识溯源、多形态生成 |
| 预审 Agent | 辩论前的双视角快速筛查 | 双视角审查、质量预估 |
| 审核纠偏 Agent | 对生成内容提出质疑与修正建议 | 幻觉检测、事实核查 |
| 裁判 Agent | 独立裁决辩论结果 | 中立判决、修正回归验证 |
| 试题生成 Agent | 按难度生成分阶试题 | 选择题/判断题/实操题 |
| 决策调度 Agent | 协调流程，根据反馈动态调整 | 流程编排、难度自适应 |
...
| Node.js | 18+ | 前端构建环境 |
| pnpm | 8+ | 前端包管理器（推荐） |
| MySQL | 8.0+ | 关系数据库（可选，也可用 SQLite） |
| Redis | 7.0+ | 缓存与会话管理（可选，不可用时自动降级为内存存储） |
| Git | 2.0+ | 版本管理 |
...
应用启动时会自动通过 SQLAlchemy 创建表结构。也可以手动执行 Alembic 迁移：
...
4. 依次体验「学情诊断 → Agent 协同 → 资源展示 → 答题反馈 → 分析报告」完整流程
...
| POST | `/api/generate` | 否 | 触发资源生成工作流 |
| GET | `/api/resources/{session_id}` | 否 | 获取生成的资源 |
| POST | `/api/feedback/` | 否 | 提交答题反馈 |
| POST | `/api/feedback/practical` | 否 | 提交实操题批改 |
| GET | `/api/questions/{session_id}` | 是 | 获取动态生成的试题 |
| GET | `/api/visualization/{session_id}` | 否 | 可视化数据聚合 |
